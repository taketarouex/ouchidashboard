...
func TestOuchi_GetTemperature(t *testing.T) {
	...
	t.Run("success no option", func(t *testing.T) {
		repository.EXPECT().fetch("test", start, end, 0, enum.Asc).Return(expectedLogs, nil)
	...
	t.Run("success set limit", func(t *testing.T) {
		repository.EXPECT().fetch("test", start, end, 3, enum.Asc).Return(expectedLogs[0:2], nil)
	...
	t.Run("success set order", func(t *testing.T) {
		...
		repository.EXPECT().fetch("test", start, end, 0, enum.Desc).Return(reversedLogs, nil)
	...
	t.Run("fail to fetch", func(t *testing.T) {
		repository.EXPECT().fetch("test", start, end, 0, enum.Asc).Return(nil, errors.New("failed to fetch"))
